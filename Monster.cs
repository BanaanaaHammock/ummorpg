...
public class Monster : Entity {
    ...
    [Header("Block")]
    [SerializeField, Range(0, 1)] float _block = 0;
    public override float block { get { return _block; } }

    [Header("Critical")]
    [SerializeField, Range(0, 1)] float _crit = 0;
    public override float crit { get { return _crit; } }
    ...
    string UpdateServer_IDLE() {
        ...
        if (EventMoveRandomly()) {
            ...
            var circle2D = Random.insideUnitCircle * moveDist;
            var circle3D = new Vector3(circle2D.x, start.y, circle2D.y);
            agent.stoppingDistance = 0;
            agent.destination = start + circle3D;
            return "MOVING";
        }
    ...
    public override void OnAggro(Entity entity) {
        ...
        if (hp > 0 && entity != null && entity.hp > 0 && CanAttackType(entity.GetType())) {
            ...
            if (target == null) {
                ...
                float distOld = Vector3.Distance(transform.position, target.transform.position);
                float distNew = Vector3.Distance(transform.position, entity.transform.position);
                if (distNew < distOld * 0.8) target = entity;
            }
        }
    }
