    ...
    void Update() {
        ...
        if (player.target != null && player.target is Npc &&
            ...
            for (int i = 0; i < questsAvailable.Count; ++i) {
                ...
                int npcIdx = Array.FindIndex(npc.quests, q => q.name == questsAvailable[i].name);

                // find quest index in player quest list
                int idx = player.GetQuestIndexByName(npc.quests[npcIdx].name);
                if (idx != -1) {
