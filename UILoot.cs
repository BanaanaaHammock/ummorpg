        ...
        if (panel.activeSelf &&
            ...
            for (int i = 0; i < items.Count; ++i) {
                ...
                if (items[i].valid) {
                    ...
                    int itemIndex = mob.lootItems.FindIndex(item => item.valid && item.name == items[i].name); // real item list index
                    entry.GetComponent<Button>().interactable = player.InventoryCanAddAmount(items[i].template, items[i].amount);
